...
5. **系统文件清理**：
   ...
   - 使用 .fnpackignore 文件排除不必要的文件（如果 fnpack 支持）
6. **错误处理**：添加适当的错误检查和日志输出，确保打包过程可追踪
...
4. **系统文件清理**：
   - 脚本会自动清理以下系统文件：
     - macOS: `.DS_Store`、`._*`（资源分支文件）
     - Windows: `Thumbs.db`、`desktop.ini`
   - 这些清理操作在两个时机执行：
     - 复制文件前清理目标目录
     - 打包前清理整个 techfunway.bookmarks 目录
   - .fnpackignore 文件已创建，包含完整的排除规则
