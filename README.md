...
本应用完全免费、无广告、开源，你的书签数据始终存储在你自己的设备上。

如果它帮到了你，欢迎请作者喝杯咖啡——**金额随意，1 元也是心意**。当然，不赞赏也完全可以，所有功能不受任何影响，项目会持续维护。

<p align="center"><img src="static/img/donate-wechat.png" alt="微信收款码" width="240" /></p>
...
- 🔍 **搜索功能** - 支持按网址名称搜索书签
- 💾 **配置保存** - 书签、排序和大部分显示配置按账号保存；部分主题、移动端视图等浏览器本地偏好仅在当前浏览器中保存

## 多用户、权限与公共书签

项目支持在同一个实例中创建多个用户。书签树和用户配置使用账号维度隔离，适合部署在飞牛 NAS 上供家人或朋友共同使用。需要注意：这是同一个 SQLite 数据库中的逻辑隔离，不是为每个用户创建独立数据库文件。

### 当前行为

| 问题 | 当前行为 |
| --- | --- |
| 文件夹、书签和排序是否独立 | 是。普通接口会按当前登录用户的 `user_id` 查询、创建、修改、删除和排序，用户 A 无法通过普通接口读取或操作用户 B 的私有数据。 |
| 主题、背景、页面布局等配置是否独立 | 不完全一致。显示开关、每行数量、默认首页、文件夹视图缩放和桌面端背景等配置保存在用户级 `sys_config` 中；部分主题、移动端视图/背景和模板排序仍保存在浏览器 `localStorage`，同一浏览器切换账号时可能沿用本地偏好。 |
| 浏览器扩展同步范围 | 是。扩展使用 `X-API-Key`，API Key 先解析到所属用户，随后所有同步查询和写入都限制在该用户的数据范围内。不要在多个账号之间共用 API Key。 |
| 管理员能否操作其他用户数据 | 可以。管理员后台可以查看指定用户的书签树、修改或删除节点、为用户创建节点、调整排序，并导出指定用户的 JSON 数据。相关操作受管理员接口保护。 |
| 免登录模式的数据归属 | 未携带有效 Token 的访客会以当前第一个启用的管理员账号访问和修改数据；所有未登录访客看到的是同一份管理员数据。免登录模式不等于为每位访客创建临时账号。携带有效 Token 的用户仍会使用自己的数据。 |

### 公共书签与共享收藏夹的边界

当前已经支持“公开书签”：书签可以设置为 `public`，并通过“公开门户”、`/public-bookmarks.html` 和 `/api/public-tree` 对外展示。公开内容不是用户私有树的一部分，其他用户不能因此获得该书签的编辑或删除权限。

当前还不支持真正的“公共收藏夹/共享文件夹”模型：

- 文件夹仍归属于创建它的用户，不能把整个文件夹授权给多个用户共同维护；
- 管理员可以为指定用户预置书签，也可以把书签设为公开，但这不是一个所有用户共同挂载的公共目录；
- 暂无把公开书签一键复制到当前用户收藏夹的接口或页面，用户需要手动重新添加，或由管理员分别预置副本；
- 因此下面这种“管理员维护一份公共收藏夹、用户各自拥有私人收藏夹”的结构目前尚未完整实现：

```text
公共收藏夹（当前版本：仅支持公开书签展示，不支持共享目录）
├── 常用工具
├── 公司系统
└── 推荐网站

用户 A / 用户 B：各自独立的私人收藏夹
```

公共收藏夹、指定文件夹共享、公共书签复制等功能可作为后续版本的独立权限模型设计；在实现前不会把现有的“公开书签”描述为“共享收藏夹”，以免造成权限预期误解。
...
### v2.1.0
- 1.更新：浏览器同步插件支持同时同步多个设备，每个设备独立配置设置
- 2.新增：免登录功能（开启免登录后实际使用的是管理员数据）
- 3.新增：新增分类显示页面，支持滑动缩放图标大小
